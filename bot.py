...
class YamCommands(commands.Cog):
    ...
    def __init__(self, bot, yam_client):
        """
        Конструктор класса.
        :param bot: главный класс(YamBot), для которого предназначены функции YamCommands.
        :param yam_client: клиент Яндекс Музыки.
        """
        self.bot = bot
    ...
    async def play(self, ctx, *search_input):
        ...
        :param search_input: Название трека, по которому выполняется поиск.
        :param ctx: Класс Context из discord.py, содержащий информацию о боте.
        """
    ...
    async def play_artist(self, ctx, *artist_name):
        ...
        :param artist_name: имя искомого музыкального исполнителя.
        :param ctx: Класс Context из discord.py, содержащий информацию о боте.
        """
    ...
    async def skip(self, ctx, skip_amount=''):
        ...
        :param ctx: Класс Context из discord.py, содержащий информацию о боте.
        :param skip_amount: Количество пропускаемых треков. Может быть как числом, так и интервалом(3-7)
        """
        if '-' in skip_amount:
